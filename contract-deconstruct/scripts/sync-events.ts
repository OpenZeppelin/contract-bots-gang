...
import * as eventJSON from '../src/static/events.json';
import { writeFileSync } from "fs";
import { join } from "path";

const fileLocation = join('src', "static", "events.json");

interface JSONInterface {
    [key: string]: Object|number|string|Array<any>;
}

let dumpCopy: JSONInterface = { ...eventJSON };

const endpoint = (page: number) =>
  `https://www.4byte.directory/api/v1/event-signatures/?ordering=created_at&page=${page}`;

const loop = async() => {
    var pagesPerIteration = 2;
    ...
    while (true) {
        ...
        var currentID = dumpCopy.latestID ? dumpCopy.latestID as number : 0;
        if(currentPage > 1 && currentID == 0) currentPage = 1; // Bad exit on latest iteration
